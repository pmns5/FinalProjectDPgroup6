class ManageUser {
    ...
    renderGUI(data) {
        ...
        $.each(data, function (index, obj) {
            ...
            if(obj.id_user !== getCookieID()) {
                ...
                if (obj.role === 'manager') {
                    row = row.replace(/{ROLE_INT}/ig, 0)
                } else {
                    row = row.replace(/{ROLE_INT}/ig, 1)
                }
                row = row.replace(/{BAN_INT}/, obj.ban)
                if (obj.ban === 1) {
                    row = row.replace(/{BAN}/ig, 'Banned');
                } else {
                    row = row.replace(/{BAN}/ig, 'Regular');
                }
                row = row.replace(/{ID_USER_B}/ig, obj.id_user)
                row = row.replace(/{ID_USER_R}/ig, obj.id_user)
    ...
    toggleBan( ban, id_user){
        ...
        $.post(this.toggleBanEndPoint, {id_user:id_user, ban:ban}, function () {
        }).done(function () {
